...
public class CalculatorImpl implements Calculator {
	...
	private List<Pair<String, String>> tokenize(String expression) {
		...
		for (int index = 0; index < exp.length; index++) {
			...
			boolean hasMinus = (exp[index] == '-')
					&& Character.isDigit(exp[index + 1]);
			...
			if (isCurrentSymbolDigit || hasMinus) {
				...
				for (; index < exp.length
						&& (Character.isDigit(exp[index])
								|| expression.charAt(index) == '.' || expression
								.charAt(index) == '-'); index++) {
		...
		for (Pair<String, String> token : tokens) {
			...
			if (type.equals(NUMBER_TYPE)) {
				...
				while (operators.size() != 0
						&& priorities.get(value) <= priorities.get(operators
								.peek())) {
