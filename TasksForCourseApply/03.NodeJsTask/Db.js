...
Db.prototype.updateById = function (id, item, done) {
    ...
    this.readItems(function (error, data) {
        if (error) {
            return done(error);
        }
        var itemToUpdate = findById(id, data);
        ...
        self.writeItems(data, function (error) {
            if (error) {
                return done(error);
            }
            done(null, itemToUpdate);
        });
    });
};
...
Db.prototype.deleteById = function (id, done) {
    ...
    this.readItems(function (error, data) {
        ...
        var itemToDelete = findById(id, data);

        if (!itemToDelete) {
            done('Item not found');
        }

        var itemToDeleteIndex = data.indexOf(itemToDelete);
